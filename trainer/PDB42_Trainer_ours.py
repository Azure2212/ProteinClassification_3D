...
class NeighborSmoothingLoss(nn.Module):
    """
    Label smoothing where the (1 - confidence) mass is split ONLY across each
    class's known sequence-similarity neighbors, instead of uniformly across
    every other class.

    For a true class T with K neighbors and smoothing eps:
        target[T]              = 1 - eps
        target[neighbor_i]     = eps / K   (for each of K neighbors)
        target[everything else] = 0

    Falls back to a one-hot target (no smoothing) for classes that have no
    neighbors in `neighbors_per_ids`.
    """

    def __init__(self, num_classes, smoothing, class_names, neighbors_per_ids):
        super().__init__()
        self.num_classes = num_classes
        self.smoothing   = float(smoothing)

        # class_names: {idx: pdb_id}  ->  pdb_id: idx
        pdb2idx = {pid: idx for idx, pid in class_names.items()}

        targets = torch.zeros(num_classes, num_classes)
        no_nbr_classes = 0

        for idx, pdb in class_names.items():
            raw = neighbors_per_ids.get(pdb, []) if neighbors_per_ids else []
            # Accept either list of IDs or {id: score} dict
            nbr_ids = list(raw.keys()) if isinstance(raw, dict) else list(raw)

            # Keep only neighbors that are actually in the training class set
            nbr_idxs = [pdb2idx[n] for n in nbr_ids if n in pdb2idx and pdb2idx[n] != idx]

            if len(nbr_idxs) > 0 and self.smoothing > 0:
                targets[idx, idx] = 1.0 - self.smoothing
                share = self.smoothing / len(nbr_idxs)
                for ni in nbr_idxs:
                    targets[idx, ni] = share
            else:
                targets[idx, idx] = 1.0
                no_nbr_classes += 1

        # register_buffer keeps the tensor on the model's device after .to(device)
        self.register_buffer("targets", targets)
        print(f"NeighborSmoothingLoss: smoothing={self.smoothing}, "
              f"{num_classes - no_nbr_classes}/{num_classes} classes have neighbors "
              f"({no_nbr_classes} use one-hot)")

    def forward(self, logits, labels):
        log_probs    = F.log_softmax(logits, dim=-1)
        soft_targets = self.targets[labels]                # [B, num_classes]
...
class PDB42_Trainer:
    def __init__(self, model, device,
        ...
        if neighbors_per_ids is not None and label_smoothing > 0 and class_names is not None:
            self.loss_fn = NeighborSmoothingLoss(
                num_classes        = len(class_names),
                smoothing          = label_smoothing,
                class_names        = class_names,
                neighbors_per_ids  = neighbors_per_ids,
            ).to(device)
            print(f"Loss: NeighborSmoothingLoss(label_smoothing={label_smoothing})")
        else:
